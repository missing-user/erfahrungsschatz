...
with st.form(key="collab_input"):
    ...
    partner_email = col1.text_input(
        "Share your insights with a collaborator",
        placeholder="Enter email address",
        label_visibility="collapsed",
    )
    if col2.form_submit_button("Send", type="primary") and partner_email:
        try:
            db.child("collabrequests").child(shash(partner_email)).push(
                st.session_state["auth_user"]["idToken"]
            )
        ...
        with st.chat_message(partner_email):
            ...
            with col2:
                if st.button("Accept", key=collab_req.key() + "accept"):
                    ...
                    db.child("collaborators").child(uid).child(partner_uid).set(
                        partner_email
                    )
                    db.child("collaborators").child(partner_uid).child(uid).set(
                        partner_email
                    )
                    ...
                    with col1:
                        placeholder.success(
                            f"You are now collaborating with {partner_email}"
                        )
            with col3:
                if st.button("Decline", key=collab_req.key() + "decline"):
                    ...
                    with col1:
                        placeholder.error(
                            f"Collaboration request from {partner_email} declined"
                        )
